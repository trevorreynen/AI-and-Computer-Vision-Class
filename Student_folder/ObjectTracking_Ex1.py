...
# OpenCV provides all these in a single function, cv.calcOpticalFlowPyrLK(). Check the slides for details of parameter.
...
# Create some random colors. Used to create our trails for object movement in the image. Last parameter is the dimension of matrix.
...
# After finding intial points, we iteratively track those points.
while 1:
    ret, frame = cap.read()
    ...
    # Calculate Optical Flow. For the function cv.calcOpticalFlowPyrLK() we pass the previous frame, previous points and next frame.
    ...
    # Image Addition (tracking line and draw the circle). Calculates the per-element sum of two arrays or an array and a scalar.
